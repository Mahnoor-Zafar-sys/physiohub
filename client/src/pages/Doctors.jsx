...
const BRANCHES = ["all", "Blue Area", "F-8 Markaz", "DHA Phase 2"];
...
function Banner() {
  ...
  return (
    <section ref={ref} className="relative overflow-hidden pt-40 pb-24 text-left"
      style={{ 
        backgroundImage: `url("https://images.unsplash.com/photo-1576091160550-2173dba999ef?auto=format&fit=crop&w=1600&q=80")`,
      ...
      <div className="relative z-20 max-w-6xl mx-auto px-4 sm:px-6">
        {/* Typewriter Heading */}
        <div className="text-4xl sm:text-6xl font-extrabold text-white leading-tight tracking-tight font-serif mb-8 min-h-[5rem] sm:min-h-[7.5rem] max-w-3xl">
          <span>{tw1.split(" ").map((word, wi) => {
            ...
            return (
              <span key={wi}>
                {isGrad
                  ? <span style={{ background:"linear-gradient(90deg, #60a5fa, #f472b6)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>{word}</span>
                  : word}
                {" "}
              </span>
            );
        ...
        <motion.div initial={{ opacity:0, y:20 }} animate={inView ? { opacity:1, y:0 } : {}}
          ...
          <button onClick={() => navigate("/book-appointment")}
            className="px-8 py-3.5 rounded-xl font-bold text-sm text-white shadow-xl hover:opacity-90 transition-opacity flex items-center gap-2 cursor-pointer border-none"
            style={{ background:"linear-gradient(135deg,#2563eb,#0ea5e9)" }}>
            <FiCalendar size={15} /> Book Appointment
          </button>
          <a href="tel:+923008786187"
            className="px-8 py-3.5 rounded-xl font-bold text-sm text-white bg-white/10 border border-white/20 hover:bg-white/20 hover:shadow-md transition-all flex items-center gap-2">
...
export default function DoctorsPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-body">
      <SEOHead 
        title="Top Physiotherapists & Rehabilitation Specialists in Islamabad | Vital Physio Hub"
        description="Find top physical therapists in Islamabad (Blue Area, F-8 Markaz, DHA Phase 2). Consult verified specialists for sports injury recovery, stroke rehab, spine & back pain therapy."
        keywords="best physiotherapist Islamabad, top physical therapist Islamabad, physio doctor Blue Area Islamabad, rehabilitation specialist DHA Islamabad, sports physio doctor Islamabad"
